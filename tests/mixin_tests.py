...
class CacheKeyMixinGenerateKeyTests(TestCase):
    ...
    def test_consistent_key_generation(self, mock_sql, mock_model_cache):
        "generate_key generates identical keys for repeated calls when sql and model are the same"
    ...
    def test_new_key_generation(self, mock_uuid, mock_sql, mock_model_cache):
        "generate_key broadcasts key for the model when it is newly created"
    ...
    def test_key_components(self, mock_sql, mock_model_cache):
        "Ensure key created by generate_key is a hash of model_key, sql query and database name"
        mock_sql.return_value = 'sql'
